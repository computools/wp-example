 ...
 */
if ( ! class_exists( 'Insight_Post_Like' ) ) {
	class Insight_Post_Like {
		...
		public function process_simple_like() {
			...
			if ( ! wp_verify_nonce( $nonce, 'simple-likes-nonce' ) ) {
				exit( esc_html__( 'Not permitted', 'tm-moody' ) );
			}
		...
		public function get_simple_likes_button( $post_id, $is_comment = null ) {
			...
			if ( $this->already_liked( $post_id, $is_comment ) ) {
				...
				$title = esc_html__( 'Unlike', 'tm-moody' );
				$icon  = $icon_full;
			} else {
				$class = '';
				$title = esc_html__( 'Like', 'tm-moody' );
				$icon  = $icon_empty;
			}
		...
		function show_user_likes( $user ) { ?>
			<table class="form-table">
				<tr>
					<th><label for="user_likes"><?php esc_html_e( 'You Like:', 'tm-moody' ); ?></label></th>
					<td>
						<?php
						...
						<?php else : ?>
							<p><?php esc_html_e( 'You do not like anything yet.', 'tm-moody' ); ?></p>
							<?php
						endif;
						wp_reset_postdata();
						?>
